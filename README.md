...
<script>
    ...
    function verifyOTP() {
        ...
        let users = getUsers();
        if(!users[tempPhone]) {
            // Sabhi users (including admin) ko shuruat mein free points milenge par app chalane ke liye subscription chahiye hogi
    ...
    function loadDashboard() {
        ...
        if(hasActiveSub) {
            ...
            if(userData.subscription.includes('Match')) subText += ` (${userData.matchesLeft} Matches Left)`;
    ...
    function checkAdminAccess() {
        ...
        // Strict Check: Admin ko bhi bina active subscription ke panel nahi khulne dega
        let hasActiveSub = (userData.subscription && userData.subExpiry > Date.now()) || (userData.matchesLeft > 0);
        
        if(!hasActiveSub) {
            alert("Admin ke liye bhi active subscription hona anivarya hai! Kripya pehle Wallet se koi plan kharidein.");
            switchTab('wallet');
            return;
        }
    ...
    // Event Listeners Binding (Buttons ke kaam na karne ki problem fix karne ke liye)
